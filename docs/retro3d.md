...
### `retro.setSectorMap(largura, altura, tabela)`
Define a malha no formato de Geometria de Setores Variáveis (Estilo DOOM/Duke3D Sem Polígonos). A sua array/tabela Lua de grid não recebe apenas o "ID da Parede", você envia um **HEX Color/Number de 32 Bits** compacto (`0xBBGGRR`). O DDA na GPU é preparado para ler estes blocos complexos no Shader Vertical.
...
### `retro.shootRaycast(x, y, angulo, maxDist, listaInimigos)`
DDA de Tiro super rápido portado direto no C++. A função ataca o Grid inteiro em *microsegundos* numa varredura matemática.
Ela não só testa colisões brutas contra as paredes do mapa, como também calcula matematicamente as intersecções de feixe com Círculos, desde que os objetos fornecidos na sua tabela Lua contenham `{x, y, radius}`.
- O C++ te devolve mastigadinho 3 valores: `Tipo de Hit` (Parede = 1, Inimigo = 2), `Distância`, e `O Index do Inimigo na Tabela`. Funcionalidade indispensável para calcular armas baseadas em hitscan instantâneo perfeitamente na Engine 3D.
...
### `retro.drawEntities(tabela)`
Renderiza as sprites do jogo (inimigos, barris, colecionáveis) utilizando **Z-Buffer Totalmente Nativo (`gl_FragDepth`)**.  
A função pega a sua lista de Sprites, as joga em uma malha 3D invisível, e renderiza brutalmente via C++ acelerado pela GPU no mesmo espaço de render principal!
Se um inimigo estiver a meio passo atrás de uma parede, ele é perfeitamente *"recortado"* horizontalmente e tridimensionalmente pela própria arquitetura da GPU. **Zero uso de CPU** para ordenar o famigerado z-sorting e overlap Depth! A magia é tratada a nível do Hardware e gl_FragDepth providenciando o recorte realista de um motor 3D autêntico.
