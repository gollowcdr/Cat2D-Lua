...
## Estrutura Padrão da Engine

Todo jogo na Cat Engine deve obrigatoriamente possuir as três funções principais listadas abaixo. Elas formam a estrutura padrão do motor e são chamadas automaticamente:

```lua
-- É chamada obrigatoriamente e apenas uma vez quando o jogo ou a tela é iniciada.
-- Serve para carregar imagens, sons e configurar variáveis iniciais.
function load()
end

-- É chamada continuamente a cada quadro (frame) antes de desenhar na tela.
-- O parâmetro 'dt' (Delta Time) é obrigatório e serve para manter a velocidade do jogo constante
-- independente de o celular ser rápido ou lento. É aqui que processamos física, entrada e lógica.
function update(dt)
end

-- É chamada continuamente para desenhar os gráficos na tela após o update.
-- Serve unicamente para renderização.
function draw()
end
```
...
function load()
    -- Carrega a imagem apenas uma vez no início
    playerImage = graphics.loadTexture("player.png")
end

function update(dt)
    -- Lógica do jogo (movimentação, etc) entraria aqui
end
